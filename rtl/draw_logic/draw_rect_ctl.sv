...
always_comb begin
    ...
    case(state)
        ...
        DOWN: begin
            if (ypos < (VER_PIXELS - H_OF_REC)) begin
                ...
                state_nxt = ENDGAME; // Przejście do stanu ENDGAME, gdy ypos osiągnie VER_PIXELS - H_OF_REC
            end
    ...
    // Dodanie warunków końca gry w zależności od pozycji klocka i przeszkód
    if (((ypos_nxt >= obstacle_ypos_1) && (ypos_nxt <= obstacle_ypos_1 + H_OF_REC) && (ypos_nxt >= obstacle_xpos_1) && (ypos_nxt <= obstacle_xpos_1 + W_OF_REC)) || 
        ((ypos_nxt >= obstacle_ypos_2) && (ypos_nxt <= obstacle_ypos_2 + H_OF_REC) && (ypos_nxt >= obstacle_xpos_1) && (ypos_nxt <= obstacle_xpos_1 + W_OF_REC))) begin
        state_nxt = ENDGAME;
    end
end
