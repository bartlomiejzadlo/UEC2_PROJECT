// Author BZ
// Module containing control for drawing obstacles purposes

module draw_obstacle_ctl(
    input  logic clk,
    input  logic rst,
    input  logic [11:0] y_from_draw_rect,
    input  logic endgame_from_rect, 
...
logic endgame_from_obstacle_nxt;
...
always_ff @(posedge clk) begin
    if (rst) begin
        ...
        endgame_from_obstacle <= endgame_from_obstacle_nxt;

        if (cycle_counter < 1_800_000) begin 
    ...
    case(state)
        IDLE: begin
            if (xpos_nxt == 0) begin
                state_nxt = START;
                xpos_nxt = 974;
                obstacle_ypos_1_nxt = obstacle_ypos_1 - 20;
                obstacle_ypos_2_nxt = obstacle_ypos_2 - 20;
                endgame_from_obstacle_nxt = endgame_from_obstacle;
            end else begin
                state_nxt = IDLE;
                xpos_nxt = 0;
                endgame_from_obstacle_nxt = '0;
            end
