...
always_ff @(posedge clk) begin
    if (rst) begin
        ...
        endgame <= 0;
        collision_detected <= 0; // Reset sygnału kolizji
    end else begin
        state <= state_nxt;
...
always_comb begin
    ...
    collision_detected = 0;     // Reset sygnału kolizji w każdym cyklu
    ...
    case(state)
        ...
        START: begin
            ...
            // Sprawdzenie kolizji
            else if (((xpos_nxt >= 0) && (xpos_nxt <= W_OF_REC) && 
                ...
                collision_detected = 1;  // Wykrycie kolizji
                state_nxt = ENDGAME;
            end else begin
                state_nxt = IDLE;
                xpos_nxt = 0;
